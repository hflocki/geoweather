...
from .pollen_regions import DEFAULT_POLLEN_MAPPING
...
class GeoWeatherCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, entry) -> None:
        # Optionales Auto-Polling: 0 = deaktiviert, sonst Intervall in Minuten
        interval_min = {**entry.data, **entry.options}.get(CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL)
        from datetime import timedelta
        update_interval = timedelta(minutes=interval_min) if interval_min and interval_min > 0 else None
    ...
    async def async_load_pollen_mapping(self) -> None:
        """Lädt das Pollen-Regions-Mapping.

        Basis: pollen_regions.py (Namens-basiert, ganz Deutschland).
        Ergänzung/Überschreibung: /config/pollen_mapping.yaml des Nutzers.
        Format: "Kreisname": "DWD Regionsname"
        """
        self._pollen_mapping = dict(DEFAULT_POLLEN_MAPPING)
            ...
            def _read():
                if os.path.exists(path):
                    with open(path, "r", encoding="utf-8") as f:
                        return f.read()
                return ""
            file_content = await self.hass.async_add_executor_job(_read)
            if file_content:
                data = yaml.safe_load(file_content)
                if isinstance(data, dict):
                    self._pollen_mapping.update(data)
                    _LOGGER.debug(
                        "pollen_mapping.yaml: %d Einträge geladen (gesamt: %d)",
                        len(data), len(self._pollen_mapping)
                    )
            else:
                _LOGGER.debug(
                    "Kein pollen_mapping.yaml – nutze eingebautes Mapping (%d Einträge)",
    ...
    def _build_pollen_result(self, json_data: dict, kreis: str) -> dict:
        # Kreisname → DWD-Regionsname (immer Namens-basiert)
